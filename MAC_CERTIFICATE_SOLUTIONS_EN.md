...
## Multi-Tier SSL Configuration Approach

For robust certificate handling, implement multiple SSL configurations:

1. **Custom Certificates**: For known sites (dl.espressif.com, github.com)
2. **Default SSL Context**: Standard Python SSL
3. **Unverified SSL**: As fallback (less secure)
4. **No SSL Context**: For HTTP or as last resort

```python
# Example SSL configuration strategy:
ssl_configs = [
...
for config_name, ctx in ssl_configs:
    try:
        # Attempt download with this SSL configuration
        download_with_ssl_context(url, destination, ctx)
        break
    except Exception:
        continue
```
...
1. **Reinstall Python**: Complete clean installation
2. **Use Virtual Environment**: `python3 -m venv venv && source venv/bin/activate`
3. **Alternative Download Method**: Use `curl` or `wget` instead of Python
4. **Offline Installation**: Download files manually on another system

The multi-tier SSL approach ensures maximum compatibility across different macOS Python installations and network environments.
